...
function pRDFData(aID, aDataSourceURI, aType, aNS, aRsourceBaseURI, aIDFormat, aShouldFlushOnEdit)
{
	...
	if (this.id_format.indexOf('%itemID%') < 0) this.id_format += '%itemID%';

	var ProfD = this.getURISpecFromKey('ProfD');
	if (ProfD.search(/\/$/) < 0) ProfD += '/';
	this.dsource_uri = (aDataSourceURI && aDataSourceURI.search(/^\w+:/) > -1) ? aDataSourceURI : ProfD+aDataSourceURI ;
...
//						dump(e+'\n');
	...
	getDataFromPath : function(aPath, aKey)
	{
		...
		if (aPath.search(/^[^:\/]+:[^:\/]+:\//i) > -1 ||
			aPath.search(/^(view-source|about|jar):/i) > -1) return '';
...
//				dump(e+'\n');
	...
	cleanUp : function(aForce)
	{
		...
		while (resources.hasMoreElements())
		{
			...
			if (resource.Value.search(/:root$/) < 0 &&
